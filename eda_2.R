...
binary_modelling_dataframe_successful_pressure_summary <- successful_pressure_binary_dataframe %>%
  group_by(shots_occur) %>%
  summarise(
    successful_pressure_closest_to_opposition_goal = mean(successful_pressure_closest_to_opposition_goal, na.rm = TRUE)
  )
...
binary_modelling_dataframe_opposition_turnover_summary <- opposition_turnover_binary_dataframe %>%
  group_by(shots_occur) %>%
  summarise(
    opposition_turnover_closest_to_their_own_goal = mean(opposition_turnover_closest_to_their_own_goal, na.rm = TRUE)
  )
